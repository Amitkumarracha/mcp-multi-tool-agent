...
async def run_agent():
    ...
    currency_api_key = os.environ.get("CURRENCY_API_KEY", "")  # Optional - free tier available
    ...
    client = ProcessTrackingClient(
        {
            "news": {
                "command": sys.executable,
                "args": ["-c", f"import sys; sys.path.insert(0, '{SRC_DIR}'); from mcpserver.news import run_server; run_server()"],
                "transport": "stdio",
                "env": {
            ...
            "currency": {
                "command": sys.executable,
                "args": ["-c", f"import sys; sys.path.insert(0, '{SRC_DIR}'); from mcpserver.currency import run_server; run_server()"],
                "transport": "stdio",
                "env": {
    ...
    try:
        ...
        system_message = SystemMessage(content=(
            "You are an intelligent multi-tool agent with access to various specialized services. "
            "Use them dynamically and efficiently based on the user's request:\n"
            "- News tools: Search for news articles, get top headlines by country/category\n"
            "- Currency tools: Convert currencies, get exchange rates, compare multiple currencies\n"
            "- Tavily tools: Search the web for current information and research\n"
            "- Math tools: Evaluate mathematical expressions and perform calculations\n\n"
            "Always use the most appropriate tool for the task. Be efficient and accurate."
        ))
